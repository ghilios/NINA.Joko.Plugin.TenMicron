...
namespace NINA.Joko.Plugin.TenMicron.SequenceItems {
    ...
    [ExportMetadata("Description", "Powers off the 10u mount by safely shutting it down")]
    ...
    public class Shutdown : SequenceItem, IValidatable {
        ...
        public Shutdown() : this(TenMicronPlugin.MountMediator) {
        }
        ...
        public override Task Execute(IProgress<ApplicationStatus> progress, CancellationToken token) {
            if (mountMediator.GetInfo().Connected) {
                if (!mountMediator.Shutdown()) {
                    throw new Exception("Failed to shutdown the 10u mount");
                }
            }
